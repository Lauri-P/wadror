class BreweriesController < ApplicationController
  ...
  def index
    ...
    direction=1
    if not session[:last_direction]
      session[:last_direction]=1
    end

    if session[:last_order]==@order
      direction=-1*session[:last_direction]
    end
    ...
    @active_breweries = case @order
                          ...
                          when 'year' then @active_breweries.sort_by{ |b| direction*b.year }
                        end

    @retired_breweries = case @order
                           when 'name' then @retired_breweries.sort_by{ |b| (b.name) }
                           when 'year' then @retired_breweries.sort_by{ |b| direction*b.year }
                         end

    if direction<0 and @order=='name'
